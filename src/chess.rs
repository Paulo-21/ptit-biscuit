...
pub static MVV_LVA: [[u16; 6]; 6] = [
    [500, 400, 300, 200, 100, 0],
    [501, 401, 301, 201, 101, 1],
    [502, 402, 302, 202, 102, 2],
    [503, 403, 303, 203, 103, 3],
    [504, 404, 304, 204, 104, 4],
    [505, 405, 305, 205, 105, 5],
];
...
pub fn array_to_bitboard(
    ...
    for v in chessboard {
        for c in v {
            match c {
                'p' => {
                    *wp += convert_string_to_bitboard(i);
                }
                'n' => {
                    *wn += convert_string_to_bitboard(i);
                }
                'b' => {
                    *wb += convert_string_to_bitboard(i);
                }
                'r' => {
                    *wr += convert_string_to_bitboard(i);
                }
                'q' => {
                    *wq += convert_string_to_bitboard(i);
                }
                'k' => {
                    *wk += convert_string_to_bitboard(i);
                }
                'P' => {
                    *bp += convert_string_to_bitboard(i);
                }
                'N' => {
                    *bn += convert_string_to_bitboard(i);
                }
                'B' => {
                    *bb += convert_string_to_bitboard(i);
                }
                'R' => {
                    *br += convert_string_to_bitboard(i);
                }
                'Q' => {
                    *bq += convert_string_to_bitboard(i);
                }
                'K' => {
                    *bk += convert_string_to_bitboard(i);
                }
                _ => {}
            }
            i += 1;
...
const fn rank_of(s: u64) -> u64 {
    s >> 3
}
const fn file_of(s: u64) -> u64 {
    s & 0b111
}
const fn diagonal_of(s: u64) -> u64 {
    return 7 + rank_of(s) - file_of(s);
}
const fn anti_diagonal_of(s: u64) -> u64 {
...
#[inline(always)]
pub fn get_score_move(attacker: usize, victim: usize) -> u16 {
    //print!("{} {} , ", attacker, victim);
    MVV_LVA[attacker - 1][victim - 1]
}

//pub fn sort_move(moves : &mut [u64], mut score : Vec<u64>) {
/*pub fn sort_move(moves: &mut [u64; MAX_CAPTURE], mut score: [u16; MAX_CAPTURE]) {
    for i in score.len()..1 {
        let mut j = i;
...
pub fn _get_pinned_b(game: &Game) -> u64 {
    ...
    let mut pinned = 0;
    let mut pinner =
        xray_rook_attacks(occupied_bb, own_pieces, square_of_king) & (game.wr | game.wq);
...
pub fn _get_pinned_w(game: &Game) -> u64 {
    ...
    let mut pinned = 0;
    let mut pinner =
        xray_rook_attacks(occupied_bb, own_pieces, square_of_king) & (game.br | game.bq);
    //_draw_bitboard(pinner);
    while pinner != 0 {
        let sq = pinner.tzcnt();
        pinned |= REC_TABLE[sq as usize][square_of_king as usize] & own_pieces;
...
pub fn get_checked_mask_b(game: &Game) -> u64 {
    ...
    while copy_wb != 0 {
        let copy_wb_tzcnt = copy_wb.tzcnt();
        let attack = diag_antid_moves(copy_wb_tzcnt, occupied);
        if attack & k != 0 {
            if checked_mask != full {
                return 0;
            }
            checked_mask &=
                REC_TABLE[copy_wb_tzcnt as usize][k.tzcnt() as usize] | (1 << copy_wb_tzcnt)
        }
        copy_wb = copy_wb.blsr();
    }
    let mut copy_wr = game.wr | game.wq;
    while copy_wr != 0 {
        let copy_wr_tzcnt = copy_wr.tzcnt();
        let attack = hv_moves(copy_wr_tzcnt, occupied);
        if attack & k != 0 {
            if checked_mask != full {
                return 0;
            }
            checked_mask &=
                REC_TABLE[copy_wr_tzcnt as usize][k.tzcnt() as usize] | (1 << copy_wr_tzcnt)
        }
...
pub fn get_legal_moves_fast_c(
    ...
    let mut score_capture = [(0u16, 0u64); MAX_CAPTURE];
    ...
    if game.white_to_play {
        ...
        while p != 0 {
            ...
            while att != 0 {
                ...
                let piece = 1 << b;
                if 1 << b & black != 0 {
                    let victim = ((piece & game.bp) != 0) as u64
                        + (piece & game.bn != 0) as u64 * 2
                        + (piece & game.bb != 0) as u64 * 3
                        + (piece & game.br != 0) as u64 * 4
                        + (piece & game.bq != 0) as u64 * 5
                        + (piece & game.bk != 0) as u64 * 6;
                    score_capture[c_i] = (
                        get_score_move(5, victim as usize),
                        (p_tzcnt << 9) + (b << 1),
                    );
        ...
        while p2 != 0 {
            let p2_tzcnt = p2.tzcnt();
            let mut att = diag_antid_moves(p2_tzcnt, occupied) & !white & checkmask & pin_d12;
            ...
            while att != 0 {
                ...
                if 1 << b & black != 0 {
                    ...
                    legal_moves[q_i] = (p2_tzcnt << 9) + (b << 1);
                    q_i += 1;
                }
                att = att.blsr();
            }
            ...
            while att != 0 {
                ...
                if 1 << b & black != 0 {
                    let victim = ((piece & game.bp) != 0) as u64
                        ...
                        + (piece & game.bk != 0) as u64 * 6;
                    score_capture[c_i] = (
                        get_score_move(4, victim as usize),
                        (p_tzcnt << 9) + (b << 1),
                    );
                    c_i += 1;
                } else {
            ...
            while att != 0 {
                ...
                if 1 << b & black != 0 {
                    let victim = ((piece & game.bp) != 0) as u64
                        ...
                        + (piece & game.bk != 0) as u64 * 6;
                    score_capture[c_i] = (
                        get_score_move(3, victim as usize),
                        (p_tzcnt << 9) + (b << 1),
                    );
                    c_i += 1;
                } else {
        ...
        while copy != 0 {
            ...
            while att != 0 {
                ...
                if 1 << b & black != 0 {
                    ...
                    score_capture[c_i] = (
                        get_score_move(2, victim as usize),
                        (copy_tzcnt << 9) + (b << 1),
                    );
                    c_i += 1;
                } else {
                    legal_moves[q_i] = (copy_tzcnt << 9) + (b << 1);
                    q_i += 1;
                }
                att = att.blsr();
            }
        ...
        while p_at != 0 {
            ...
            let victim = ((piece & (game.bp | game.en_passant)) != 0) as u64
                ...
                + (piece & game.bk != 0) as u64 * 6;
            score_capture[c_i] = (
                get_score_move(1, victim as usize),
                (pi_square << 9)
                    | ((pi_square + 7) << 1)
                    | (((1 << pi_square) & RANK_MASK[6]) != 0) as u64,
            );
            //score[c_i] = get_score_move(1, victim as usize);
            /*capture[c_i] = (pi_square << 9)
            | ((pi_square + 7) << 1)
            | (((1 << pi_square) & RANK_MASK[6]) != 0) as u64;*/
            c_i += 1;
            p_at = p_at.blsr();
        ...
        while p != 0 {
            let p_tzcnt = p.tzcnt();
            let mut att = (hv_moves(p_tzcnt, occupied) | diag_antid_moves(p.tzcnt(), occupied))
                & !black
                & checkmask;
            while att != 0 {
                let b = att.tzcnt();
                let piece = 1 << b;
                if 1 << b & white != 0 {
                    let victim = ((piece & game.wp) != 0) as u64
                        + (piece & game.wn != 0) as u64 * 2
                        + (piece & game.wb != 0) as u64 * 3
                        + (piece & game.wr != 0) as u64 * 4
                        + (piece & game.wq != 0) as u64 * 5
                        + (piece & game.wk != 0) as u64 * 6;
                    score_capture[c_i] = (
                        get_score_move(5, victim as usize),
                        (p_tzcnt << 9) + (b << 1),
                    );
        ...
        while p1 != 0 {
            let p1_tzcnt = p1.tzcnt();
            let mut att = hv_moves(p1_tzcnt, occupied) & checkmask & pin_hv & !black;
            while att != 0 {
                let b = att.tzcnt();
                let piece = 1 << b;
                if 1 << b & white != 0 {
        ...
        while p2 != 0 {
            let p2_tzcnt = p2.tzcnt();
            let mut att = diag_antid_moves(p2_tzcnt, occupied) & checkmask & pin_d12 & !black;
            while att != 0 {
                let b = att.tzcnt();
                let piece = 1 << b;
                if 1 << b & white != 0 {
        ...
        while p_at != 0 {
            let pi_square = p_at.tzcnt();
            let piece = 1 << (pi_square - 7);
            let victim = ((piece & (game.wp | game.en_passant)) != 0) as u64
                + (piece & game.wn != 0) as u64 * 2
                + (piece & game.wb != 0) as u64 * 3
                + (piece & game.wr != 0) as u64 * 4
                + (piece & game.wq != 0) as u64 * 5
        ...
        while p_at2 != 0 {
            ...
            score_capture[c_i] = (
                get_score_move(1, victim as usize),
                (pi_square << 9) + ((pi_square - 9) << 1)
                    | (((1 << pi_square) & RANK_MASK[1]) != 0) as u64,
            );
            //score[c_i] = get_score_move(1, victim as usize);
            /*capture[c_i] = (pi_square << 9) + ((pi_square - 9) << 1)
            | (((1 << pi_square) & RANK_MASK[1]) != 0) as u64;*/
            p_at2 = p_at2.blsr();
            c_i += 1;
        }
        while p_at3 != 0 {
            let pi_square = p_at3.tzcnt();
            legal_moves[q_i] = (pi_square << 9) + ((pi_square - 16) << 1)
                | (((1 << pi_square) & RANK_MASK[1]) != 0) as u64;
            p_at3 = p_at3.blsr();
            q_i += 1;
        }
        while p_at4 != 0 {
            let pi_square = p_at4.tzcnt();
            legal_moves[q_i] = (pi_square << 9) + ((pi_square - 8) << 1)
                | (((1 << pi_square) & RANK_MASK[1]) != 0) as u64;
            p_at4 = p_at4.blsr();
            q_i += 1;
        }
        ...
        while p != 0 {
            ...
            if 1 << b & white != 0 {
                ...
                legal_moves[q_i] = (bk_tzcnt << 9) + (b << 1);
                q_i += 1;
            }
            p = p.blsr();
        }
    ...
    (score_capture, legal_moves, c_i, q_i)
}
...
pub fn diag_antid_moves(square: u64, occupied: u64) -> u64 {
    hyperbola_quintessence(
        occupied,
        DIAG_MASKS[((square / 8) + (square % 8)) as usize],
        square,
    ) | hyperbola_quintessence(
        occupied,
        ANTIDIAG_MASKS[((square / 8) + 7 - (square % 8)) as usize],
        square,
    )
}
//#[inline(always)]
...
pub fn compute_move_w(chessmove: (u64, u64, Piece), game: &mut Game) -> i8 {
    ...
    if (game.wp & a) != 0 {
        ...
        if square_b == 2 && square_a == 4 {
            // Grand roque
            //check if the king and the rook has never move
            if game.wking_castle
                && (black | white) & (2u64.pow(1) + 2u64.pow(2)) == 0
                && possibility_b(game) & (2u64.pow(1) + 2u64.pow(2)) == 0
            {
                game.wqueen_castle = false;
                game.wking_castle = false;
                //Do grand roque
        ...
        } else if square_b == 6 && square_a == 4 {
            //Petit Roque
            if game.wqueen_castle
                && (black | white) & (2u64.pow(6) + 2u64.pow(5)) == 0
                && possibility_b(game) & (2u64.pow(6) + 2u64.pow(5)) == 0
            {
                game.wqueen_castle = false;
                game.wk &= !a;
                game.wk |= b;
...
pub fn compute_move_b_hash(chessmove: (u64, u64, Piece), game: &mut Game) -> i8 {
    ...
    if (game.bp & a) != 0 {
        ...
        if square_a == 60 && square_b == 58 {
            // && game.bking_castle && (black | white) & (2u64.pow(58) + 2u64.pow(57)) == 0 && possibility_w(game) & (2u64.pow(58) + 2u64.pow(57)) == 0 {
            //println!("Grand roque");
            game.bking_castle = false;
            game.bqueen_castle = false;
            //Do grand roque
        ...
        //check if no piece is between
        //check if square between isn't attacked
        else if square_a == 60 && square_b == 62 {
            // && game.bqueen_castle && (black | white) & (2u64.pow(61) + 2u64.pow(62)) == 0 && possibility_w(game) & (2u64.pow(61) + 2u64.pow(62)) == 0 {
            game.bking_castle = false;
            game.bqueen_castle = false;
            game.bk &= !a;
            game.bk |= b;
...
pub fn compute_move_b(chessmove: (u64, u64, Piece), game: &mut Game) -> i8 {
    ...
    if (game.bp & a) != 0 {
        ...
        if square_a == 60
            && square_b == 58
            && game.bking_castle
            && (black | white) & (2u64.pow(58) + 2u64.pow(57)) == 0
            && possibility_w(game) & (2u64.pow(58) + 2u64.pow(57)) == 0
        {
            //println!("Grand roque");
            game.bking_castle = false;
            game.bqueen_castle = false;
        ...
        //check if square between isn't attacked
            ...
            && square_b == 62
            && game.bqueen_castle
            && (black | white) & (2u64.pow(61) + 2u64.pow(62)) == 0
            && possibility_w(game) & (2u64.pow(61) + 2u64.pow(62)) == 0
        {
            game.bking_castle = false;
            game.bqueen_castle = false;
            game.bk &= !a;
...
pub fn compute_move_b_thrust(chessmove: (u64, u64, Piece), game: &mut Game) -> i8 {
    ...
    if (game.bp & a) != 0 {
        ...
        if square_a == 60 && square_b == 58
        /*&& game.bking_never_move && game.bking_castle && (black | white) & (2u64.pow(58) + 2u64.pow(57)) == 0 && possibility_w(game) & (2u64.pow(58) + 2u64.pow(57)) == 0*/
        {
            //println!("Grand roque");
            game.bking_castle = false;
            game.bqueen_castle = false;
        ...
        /*&& game.bking_never_move && game.bqueen_castle && (black | white) & (2u64.pow(61) + 2u64.pow(62)) == 0 && possibility_w(game) & (2u64.pow(61) + 2u64.pow(62)) == 0*/
        {
            game.bking_castle = false;
            game.bqueen_castle = false;
            game.bk &= !a;
...
pub fn possibility_w(game: &Game) -> u64 {
    ...
    let mut copy_wq = game.wq;
    while copy_wq != 0 {
        attack |= (hv_moves(copy_wq.tzcnt(), occupied)
            | diag_antid_moves(copy_wq.tzcnt(), occupied))
            & !white;
        copy_wq &= copy_wq - 1;
    }
    //attack |= possibility_k(game.wk) & !white;
...
pub fn possibility_b(game: &Game) -> u64 {
    ...
    let mut copy_bq = game.bq;
    while copy_bq != 0 {
        attack |= (hv_moves(copy_bq.tzcnt(), occupied)
            | diag_antid_moves(copy_bq.tzcnt(), occupied))
            & !black;
        copy_bq &= copy_bq - 1;
    }
    attack |= KING_MOVE[game.bk.tzcnt() as usize];
...
pub fn get_legal_move(side_w: bool, game: &Game) -> VecDeque<(u64, Piece)> {
    ...
    if side_w {
        ...
        if game.wq != 0 {
            ...
            let mut wq_possi =
                (hv_moves(piece, occupied) | diag_antid_moves(piece, occupied)) & !white;
            while wq_possi != 0 {
                //let (mut wp, mut wn, mut wb, mut wr, mut wq, mut wk, mut bp, mut bn, mut bb, mut br, mut bq, mut bk) = copy_bitboard(wp1, wn1, wb1, wr1, wq1, wk1, bp1, bn1, bb1, br1, bq1, bk1);
                let mut game1 = *game;
        ...
        if game.wqueen_castle
            && (black | white) & (2u64.pow(3) + 2u64.pow(2) + 2u64.pow(1)) == 0
            && possib & (2u64.pow(3) + 2u64.pow(2) + 2u64.pow(4)) == 0
        {
            legal_moves.push_back(((4 << 9) + (2 << 1), Piece::KING));
        }
        if game.wking_castle
            && (black | white) & (2u64.pow(6) + 2u64.pow(5)) == 0
            && possib & (2u64.pow(6) + 2u64.pow(5) + 2u64.pow(4)) == 0
        {
        ...
        if game.bq != 0 {
            ...
            let mut bq_possi =
                (hv_moves(piece, occupied) | diag_antid_moves(piece, occupied)) & !black;
            while bq_possi != 0 {
                //let (mut wp, mut wn, mut wb, mut wr, mut wq, mut wk, mut bp, mut bn, mut bb, mut br, mut bq, mut bk) = copy_bitboard(wp1, wn1, wb1, wr1, wq1, wk1, bp1, bn1, bb1, br1, bq1, bk1);
                let mut game1 = *game;
        ...
        if game.bqueen_castle
            && (black | white) & (2u64.pow(58) + 2u64.pow(59) + 2u64.pow(57)) == 0
            && possiw & (2u64.pow(58) + 2u64.pow(59) + 2u64.pow(60)) == 0
        {
            legal_moves.push_back(((60 << 9) + (58 << 1), Piece::KING));
        }
        if game.bking_castle
            && (black | white) & (2u64.pow(61) + 2u64.pow(62)) == 0
            && possiw & (2u64.pow(61) + 2u64.pow(62) + 2u64.pow(60)) == 0
        {
...
pub fn _print_custum_move(a_move: (u64, Piece)) {
    ...
    if a_move.0 & 1 == 1 {
        println!(
            "{}{}q {:?}",
            convert_square_to_move(a),
            convert_square_to_move(b),
            a_move.1
        );
    } else {
        println!(
