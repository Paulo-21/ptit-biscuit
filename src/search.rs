use crate::chess::*;
use std::cmp::{ max, min };
pub fn alpha_beta(game : &Game, depth : i8, mut alpha:i16, mut beta : i16, maximizingPlayer : bool, w_play:bool) -> i16 {
    let legal_move = get_legal_move(maximizingPlayer, game);
    ...
    if maximizingPlayer {
        ...
        for moveto in legal_move {
            ...
            value = max(value, alpha_beta(&game1, depth-1, alpha, beta, maximizingPlayer^true, w_play^true));
            if value > beta {
                break;
            }
